...
def calcMetaDosages(posteriors, sample, allelic_dosages):
    ...
    for m in range(len(posteriors)):
        ...
        for key, value in panels.items():
            ...
            c, d = key[1]
            meta_dosage += (
                allelic_dosages[a - 1][b - 1][sample][m] * value
    ...
    if (
        min(np.round(np.array(meta_dosages), 3)) >= 0
        and max(np.round(np.array(meta_dosages), 3)) <= 2
    ):
        return meta_dosages
    else:
        print(
            min(np.round(np.array(meta_dosages), 3)),
            max(np.round(np.array(meta_dosages), 3)),
        )
...
def calcMetaDosages_nan(posteriors, sample, allelic_dosages):
    ...
    for m, panels in enumerate(posteriors):
        ...
        if has_nan:
            ...
            s = sum(
                (d1 + d2) for d1, d2, _ in vals if not (np.isnan(d1) or np.isnan(d2))
            )
        else:
    ...
    arr = np.clip(np.array(meta_dosages), 0, 2) # ensure within bounds
    return arr.tolist()
...
def calcViterbiDosage(opt_path, sample, allelic_dosages):
    ...
    for m in range(len(opt_path)):
        ...
        c, d = allele_2
        mg = (
            allelic_dosages[a - 1][b - 1][sample][m]
            + allelic_dosages[c - 1][d - 1][sample][m]
        )
...
posteriors0 = [
    {
        ((1, 1), (1, 2)): 0.333333333333333,
        ((2, 1), (2, 2)): 0.3333333333,
        ((1, 1), (2, 2)): 0.333333333,
    },
]
sample = 0
    ...
    [
        ...
        [
            ...
            [
                [np.nan],
                [np.nan],
            ]  # Alleles dosage for the first marker (e.g., from panel 2)
        ],
    ]
)
