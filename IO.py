...
# %%
def write_vcf(samples, SNPs, outname, CHR, missing_mask, haploid=False, phased=False, all_variants = False): 
#write vcf
...
# Open a file, "example.vcf" for writing, with our "vcfh" header
    ...
    if not all_variants: 
        SNPs = SNPs[np.where(np.sum(missing_mask == 0, axis = 1))]
        print("number of SNPs present in all panels is...", len(SNPs))
        ...
        # Set dosage
            ...
            if phased: 
                ...
                try:
                    ...
                    print(f"Error assigning DS for sample '{sample}' at variant '{CHR}:{ID[1]}', alleles={r.alleles}, DS={ds}")
                    print(f"Exception: {e}")
                    raise
...
#missing mask    
    missing_bool = all_dosage.isna() | (all_dosage == ".")
    N = len(dicto) # number of targets
    K = len(dosage_list) #number of panels
    bool_idx = np.arange(0, K * N, N) #pick first sample in each panel because if it's missing in one sample in any panel it's missing in all samples
    missing_matrix = missing_bool.iloc[:, bool_idx]
...
#loop over samples
   ...
   #clip values to prevent underflow
    ...
    return SNPs, dicto, obsGLMerge, ad_clipped, missing_matrix


# %%
def read_vcfs_genK_region_old(GL_path, *DS_paths, region, outer = True, missing_handling = "flat"): 
    '''
    @GL_path is the path for the genotype likelihoods
    @DS_paths are an unspecified number of paths for the dosages of each of the K panels 
    @region is the chromosomal region
